		...
		uint16_t localPort, const IpAddress & localAddress)
{
    ...
    if(remoteAddress.is_ipv4() && (localAddress.is_any() || localAddress.is_ipv4())) {
    	...
    	sock = create_ipv6_socket(localPort, localAddress);
    } else {
...
/******************************************************************************/
int UdpClient::create_ipv6_socket(uint16_t localPort, const struct in6_addr & localAddress)
{
